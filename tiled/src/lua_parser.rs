...
impl LayerType {
    fn lua_parse_layer_type(t: &LuaTable) -> Result<LayerType, Error> {
...
impl Properties {
    fn lua_parse_properties(props: &LuaTable) -> Result<Properties, Error> {
...
impl MapMetaData {
    fn lua_parse_map_meta_data(map_table: &LuaTable) -> Result<MapMetaData, Error> {
        ...
        Ok(MapMetaData {
            ...
            properties: Properties::lua_parse_properties(map_table)?,
            orientation,
            render_order,
        })
    }
...
impl TileLayer {
    fn lua_parse_tile_layer(
        t: &LuaTable,
        llid: u32,
        tile_buffer: &[u8],
    ) -> Result<TileLayer, Error> {
        ...
        let tile_data = if !t.contains_key("data")? {
                ...
                .sequence_values::<LuaTable>()
            {
                let (chunk, tile_x, tile_y) =
                    Chunk::lua_parse_chunk(&chunk?, &encoding, &compression, tile_buffer)?;
        ...
        Ok(TileLayer {
            ...
            properties: Properties::lua_parse_properties(t)?,
            data: tile_data,
            layer_type,
            width,
            height,
...
impl DrawOrder {
    fn lua_parse_draw_order(t: &LuaTable) -> Result<DrawOrder, Error> {
...
impl Halign {
    fn lua_parse_halign(t: &LuaTable) -> Result<Halign, Error> {
...
impl Valign {
    fn lua_parse_valign(t: &LuaTable) -> Result<Valign, Error> {
...
impl Text {
    fn lua_parse_text(t_table: &LuaTable) -> Result<Text, Error> {
        ...
        Ok(Text {
            ...
            halign: Halign::lua_parse_halign(t_table)?,
            valign: Valign::lua_parse_valign(t_table)?,
            fontfamily,
        })
    }
}
...
impl Object {
    fn lua_parse_object(
        ...
        let lua_shape_res = match obj_table.get::<_, LuaString>("shape")?.to_str()? {
            "text" => LuaShapeResolution::Text(Text::lua_parse_text(obj_table)?),
        ...
        Ok(Object {
            ...
            properties: Properties::lua_parse_properties(obj_table)?,
...
impl ObjGroupType {
    fn lua_parse_obj_group_type(t: &LuaTable) -> Result<ObjGroupType, Error> {
        match t.get::<_, LuaString>("type")?.to_str()? {
...
impl ObjectGroup {
    fn lua_parse_object_group(
        objg_table: &LuaTable,
        llid: u32,
        from_obj_layer: bool,
        slab: &mut slab::Slab<Object>,
        tileset_ids: Option<&[u8]>,
    ) -> Result<(Self, Vec<(ObjectId, ObjectRef)>)> {
        ...
        for object in objg_table.get::<_, LuaTable>("objects")?.sequence_values() {
            let object = Object::lua_parse_object(&object?, from_obj_layer, tileset_ids)?;
        ...
        Ok((
            ObjectGroup {
                ...
                properties: Properties::lua_parse_properties(objg_table)?,
                draworder: DrawOrder::lua_parse_draw_order(objg_table)?,
                obj_group_type: ObjGroupType::lua_parse_obj_group_type(objg_table)?,
...
impl Animation {
    fn lua_parse_animation(t: LuaTable, tileset: u8) -> Result<Animation, Error> {
...
impl Tile {
    fn lua_parse_tile(
        ...
        let objectgroup = match tile_table.get::<_, LuaTable>("objectGroup") {
            Ok(t) => Some(ObjectGroup::lua_parse_object_group(&t, u32::MAX, false, slab, None)?.0),
            Err(_) => None,
        };
        ...
        Ok(Tile {
            ...
            animation: match tile_table.get::<_, LuaTable>("animation") {
                Ok(t) => Some(Animation::lua_parse_animation(t, tileset_num)?),
                Err(_) => None,
            },
            properties: match tile_table.get::<_, LuaTable>("properties") {
                Ok(_) => Properties::lua_parse_properties(tile_table)?,
...
impl Tileset {
    fn lua_parse_tileset(
        ...
        let filename: Option<String> = ts.get("filename")?;
        let first_gid = ts.get("firstgid")?;

        if let Some(f) = filename {
            let tileset_file = fs.open(Path::new(&(path_prefix.unwrap_or("").to_owned() + &f)))?;
            let des_json = serde_json::from_reader(tileset_file)?;
            Tileset::json_parse_tileset(&des_json, first_gid, path_prefix, tileset_number, slab, f)
        } else {
            let mut tiles = HashMap::new();
            for tile_table in ts.get::<_, LuaTable>("tiles")?.sequence_values() {
                let tile = Tile::lua_parse_tile(&tile_table?, tileset_number, slab)?;
            ...
            Ok(Tileset {
                ...
                images: vec![Image::from_lua(ts, path_prefix)?],
                tilecount: ts.get("tilecount")?,
                properties: Properties::lua_parse_properties(ts)?,
...
impl Map {
    pub fn lua_parse_map(
        ...
        let meta_data = MapMetaData::lua_parse_map_meta_data(&tiled_lua_table)?;
        ...
        for (tileset, i) in tiled_lua_table
            ...
            .zip(0..)
        {
            let tileset = Tileset::lua_parse_tileset(&tileset?, path_prefix, i, &mut obj_slab, fs)?;
            tile_buffer.reserve(tileset.tilecount as usize);
            for _ in tileset.first_gid..tileset.first_gid + tileset.tilecount {
                tile_buffer.push(i);
            }
        ...
        {
            let layer = layer?;
            let layer_type = LayerType::lua_parse_layer_type(&layer)?;
            match layer_type {
                LayerType::Tile => {
                    let tile_layer =
                        TileLayer::lua_parse_tile_layer(&layer, tile_llid, &tile_buffer)?;
                    tile_layer_map.insert(tile_layer.name.clone(), tile_layer.id);
                    tile_layers.push(tile_layer);
                    tile_llid += 1;
                }
                LayerType::Object => {
                    let (obj_group, obj_ids_and_refs) = ObjectGroup::lua_parse_object_group(
                        &layer,
                        obj_llid,
                        true,
                        &mut obj_slab,
