...
pub trait UserData: Sized {
    ...
    fn on_metatable_init(_table: TypedAlchemyTable<Self>) {}

    /// Adds custom fields to the *type* object for this userdata.
    fn add_type_fields<'lua, F: UserDataFields<'lua, TypedAlchemyTable<Self>>>(_fields: &mut F)
    where
        Self: 'static,
    {
    }
    ...
    fn add_type_methods<'lua, M: UserDataMethods<'lua, TypedAlchemyTable<Self>>>(_methods: &mut M)
    where
        Self: 'static,
    {
    }
    ...
    fn on_type_metatable_init(_table: TypedAlchemyTable<TypedAlchemyTable<Self>>) {}
}
...
impl<'lua> AnyUserData<'lua> {
    ...
    pub fn alchemy_table(&self) -> Option<&'static AlchemyTable> {
        let lua = self.0.lua;
        unsafe {
